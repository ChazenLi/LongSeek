...
try:
    from CA_project.preprocess.extract_observations import (
    ...
    # Test with old parameters (no largest component extraction)
    wound_old = binary_to_wound_mask(
        test_mask, 
        min_hole=2000, 
        min_object_size=500,
        morph_cleanup=False
    )
    print(f"\nOld method wound area: {np.sum(wound_old)}")
    ...
    print("\n✓ Test 2 PASSED: Improved wound_area definition working")
...
try:
    from CA_project.multi_dataset_support import (
        MultiDatasetManager,
        DatasetConfig,
    ...
    print("\n✓ Test 3 PASSED: Multi-dataset support working")
...
try:
    from CA_project.ca.model_optimized import CellOnlyCAOptimized, NUMBA_AVAILABLE
    from CA_project.ca.model import CAParams
    ...
    if ms_per_step < 50:
        ...
        print(f"\n✓ Good performance: <100ms/step")
    else:
        print(f"\n⚠ Performance below target: {ms_per_step:.1f}ms/step")
    
    print("\n✓ Test 4 PASSED: JIT acceleration working")
...
try:
    from CA_project.ca.model_optimized import CellOnlyCAOptimized
    from CA_project.ca.model import CAParams
    from CA_project.inference.fit_bo import calculate_loss
    ...
    print("\n✓ Test 5 PASSED: Integration with frame weights working")
...
✓ [HIGH PRIORITY] Scientific Rigor Improvements:
  1. Frame weights from segmentation quality (w_t = clip(F_t, 0.2, 1.0))
  2. Noise model (σ_t ∝ FP_t + FN_t)
...
✓ [MEDIUM PRIORITY] Generalization Improvements:
  7. TScratch dataset support (4 locations × 2 conditions)
